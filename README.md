# Payment History Service

Микросервис для управления историей платежей с использованием:
- Java 21
- Spring WebFlux (реактивный стек)
- R2DBC (реактивный доступ к БД)
- PostgreSQL
- Docker
- Prometheus + Grafana для мониторинга

## 🚀 Быстрый старт


# 1. Клонировать проект
```git clone <repository-url>```  

```cd payment-history-service```
# 2. Установить зависимости (только при первом запуске)
```make setup```

# 3. Запустить проект
```make all```
# Запуск теста k6
``` make test-full```
## Архитектура

Проект состоит из:
1. **Mock Service** - заглушка, возвращающая случайные платежи с задержкой 200мс
2. **Main Service** - основное приложение с API для получения и сохранения платежей
3. **База данных** - PostgreSQL для хранения истории платежей
4. **Мониторинг** - Prometheus + Grafana для метрик

## 📋 Требования

* Docker + Docker Compose (обязательно)  
* curl (обязательно)  
* k6 (рекомендуется, для нагрузочного тестирования)

## 🛠 Команды
* make setup      # Проверить и установить зависимости
* make all        # Полный цикл: сборка → запуск → тест
* make build      # Собрать проект
* make up         # Запустить сервисы
* make test-api   # Проверить API
* make test-read  # Тест чтения (требует k6)
* make test-write # Тест записи (требует k6)
* make test-full  # Полное тестирование нагрузки (требует k6)
* make logs       # Просмотр логов
* make clean      # Очистка


